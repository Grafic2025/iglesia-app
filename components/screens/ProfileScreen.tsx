...
const ProfileScreen = ({ navigateTo, pickImage }: any) => {
    ...
    const { memberId, nombre, apellido, fotoUrl, fechaNacimiento, zona, rachaUsuario, asistenciasDetalle, refreshData, esServidor, deleteAccount } = useApp();
    ...
    const handleDeleteAccount = () => {
        Alert.alert(
            '⚠️ Eliminar cuenta',
            '¿Estás seguro? Se borrarán todos tus datos, asistencias, notas y foto. Esta acción no se puede deshacer.',
            [
                { text: 'Cancelar', style: 'cancel' },
                {
                    ...
                    onPress: () => {
                        Alert.alert(
                            '🚨 Confirmación final',
                            'Vas a perder acceso permanente a la app. ¿Continuar?',
                            [
                                { text: 'No, volver', style: 'cancel' },
                                {
                                    ...
                                    onPress: async () => {
                                        try {
                                            await deleteAccount();
                                            Alert.alert('Cuenta eliminada', 'Tus datos fueron borrados. ¡Que Dios te bendiga! 🙏');
                                        } catch (e: any) {
        ...
        <View style={styles.mainWrapper}>
            ...
            >
                ...
                <View style={styles.dangerZone}>
                    <TouchableOpacity style={styles.deleteButtonProfile} onPress={handleDeleteAccount} activeOpacity={0.8}>
                        <MaterialCommunityIcons name="account-remove" size={18} color="#ff4444" />
...
const styles = StyleSheet.create({
    ...
    statLabel: { color: '#666', fontSize: 10, marginTop: 4, fontWeight: '700', letterSpacing: 1 },
    ...
    sectionTitle: { color: '#999', fontSize: 11, fontWeight: '900', letterSpacing: 2 },
    ...
    saveBtnText: { color: '#000', fontFamily: 'Montserrat_900Black', fontSize: 13, letterSpacing: 1 },

    dangerZone: { marginTop: 15, marginBottom: 50, alignItems: 'center' },
    deleteButtonProfile: { flexDirection: 'row', alignItems: 'center', backgroundColor: 'rgba(255, 68, 68, 0.05)', paddingVertical: 14, paddingHorizontal: 22, borderRadius: 20, borderWidth: 1, borderColor: 'rgba(255, 68, 68, 0.15)', marginBottom: 20 },
    deleteButtonText: { color: '#ff4444', fontFamily: 'Montserrat_700Bold', fontSize: 13, marginLeft: 10, letterSpacing: 0.5 },
    privacyLink: { padding: 10 },
    privacyLinkText: { color: '#888', fontSize: 11, fontWeight: '600', textDecorationLine: 'underline' },
});
